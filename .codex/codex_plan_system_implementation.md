...
Plan:
1. [status: completed] Level Loading | Phase 1: Add typed level data models and `Resources` provider for `Levels.json`.
...
2. [status: completed] Level Loading | Phase 2: Integrate typed data into `LevelLoader`, `CoinSpawner`, `LevelManager`, and level progression.
...
3. [status: completed] Enemy AI | Phase 1: Define behavior architecture (`IEnemyState`, state machine context, transition rules for Patrol/Chase/Attack).
